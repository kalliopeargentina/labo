...
AgregarVariables_IntraMes  <- function( dataset )
{
  ...
  dataset[  , operaciones_vs_calcenter  := (ctarjeta_debito+ ctarjeta_debito_transacciones +
   ctarjeta_visa_transacciones + ctarjeta_master_transacciones + cpayroll2_trx + ccuenta_debitos_automaticos +
   ctarjeta_visa_debitos_automaticos+ ctarjeta_master_debitos_automaticos + cpagodeservicios + cpagomiscuentas +
   cforex + cforex_buy + cforex_sell + mforex_sell + ctransferencias_recibidas + ctransferencias_emitidas +
   cextraccion_autoservicio + ccheques_depositados + ccheques_emitidos + ccheques_depositados_rechazados + 
   ccheques_emitidos_rechazados  + chomebanking_transacciones +
   ccajas_transacciones + ccajas_consultas + ccajas_depositos + ccajas_extracciones + ccajas_otras + 
   catm_trx + catm_trx_other + ctrx_quarter + cmobile_app_trx +
   Master_cconsumos + Master_cadelantosefectivo +  
   Visa_cconsumos  + Visa_cadelantosefectivo ) / ccallcenter_transacciones ]
   #proporción de transferencias sobre el capital total
   dataset[ , flujo_vs_capital    := (mtransferencias_recibidas - ctransferencias_emitidas)  / mcuentas_saldo ]
